        ...
        <NavbarMenu>
          {menuItems.map((item, index) => (
            <NavbarMenuItem key={`${item}-${index}`}>
              {index === 2 ? ( // 当索引为2（即"Clubs"）时，添加箭头按钮
                <>
                  <Link className="w-full" color="warning" href="#" size="lg">
                    {item}
                  </Link>
                  <DropdownMenu
                    aria-label="Clubs submenu"
                    align="right"
                    itemClasses={{
                      base: "gap-4",
                    }}
                  >
                    {/* 添加 "Analytics", "System", "Deployments", "My Settings", "Team Settings", "Help & Feedback" */}
                    {/* ... */}
                  </DropdownMenu>
                </>
              ) : (
